...
if ( !empty($dateMin)) {
	$listeIdProduit = array();
	// connexion à la base de données
	include 'connexion.php';
	if (isset($datMin)) {
		
		$requete = 'SELECT id_produit, nom_produit FROM produits ORDER BY id_propriete';
		// exécution de la requête
		$resultat = $connexion -> query($requete) or die(print_r($connexion -> errorInfo()));
		// résultats
		while ($donnees = $resultat -> fetch(PDO::FETCH_ASSOC)) {
			// je remplis un tableau et mettant l'id en indexvb
			$headerLigne[utf8_encode($donnees['value'])]= utf8_encode($donnees['value']);
			$ligne++;
		}
